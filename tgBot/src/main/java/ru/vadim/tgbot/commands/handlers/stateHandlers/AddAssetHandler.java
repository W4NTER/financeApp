...
import static ru.vadim.tgbot.utils.constants.Constants.ASSET_TYPE;

@Component
@AllArgsConstructor
public class AddAssetHandler implements StateHandler {
    private final LiabilitiesAssetsWebClient liabilitiesAssetsWebClient;

    @Override
    public boolean supports(String state) {
        return StateType.ADD_ASSET.toString().equals(state);
    }
